...
class Visualization(object):
	...
	def __init__(self, sar_data, cpu=True, mem=True, paging=False, disk=False, network=False):
		...
		if not isinstance(sar_data, dict):
            raise 'Incompatible sar_data type: {}'.format(type(sar_data).__name__)
		...
		self.time_points = []
		"""(:obj:`list` of :obj:`str`): time points which system activity was recorded"""
	...
	def _preprocess_sar_data(self):
		...
		if self.enable_cpu:
			self.cpu_usage_sys = [self.sar_data['cpu'][tp]['all']['sys'] for tp in self.time_points]
			self.cpu_usage_usr = [self.sar_data['cpu'][tp]['all']['usr'] for tp in self.time_points]

		if self.enable_mem:
			self.pct_mem_used = [self.sar_data['mem'][tp]['memusedpercent'] / 1024 for tp in self.time_points]
			self.mem_used_mb = [(self.sar_data['mem'][tp]['memused'] - (self.sar_data['mem'][tp]['memcache'] + self.sar_data['mem'][tp]['membuffer'])) / 1024 for tp in self.time_points]
			self.mem_cached_mb = [self.sar_data['mem'][tp]['memcache'] / 1024 for tp in self.time_points]
			self.mem_buffer_mb = [self.sar_data['mem'][tp]['membuffer'] / 1024 for tp in self.time_points]

		if self.enable_paging:
			self.page_faults_per_sec = [self.sar_data['paging'][tp]['fault'] for tp in self.time_points]
			self.major_page_faults_per_sec = [self.sar_data['paging'][tp]['majflt'] for tp in self.time_points]
			self.page_ins_per_sec = [self.sar_data['paging'][tp]['pgpgin'] for tp in self.time_points]
			self.page_outs_per_sec = [self.sar_data['paging'][tp]['pgpgout'] for tp in self.time_points]
	...
	def save(self, output_path, output_type=PDF_OUTPUT):
        ...
        if self.enable_cpu:
        	...
        	plt.xticks(self.xticks, self.xtick_labels, rotation=Visualization.PLT_XTICK_LABEL_ROTATION)
        ...
        if self.enable_mem:
        	...
        	plt.subplot(self.num_plots, 1, plt_idx)
        	plt.xticks(self.xticks, self.xtick_labels, rotation=Visualization.PLT_XTICK_LABEL_ROTATION)
        	plt.stackplot(self.x_data, self.mem_buffer_mb, self.mem_cached_mb, self.mem_used_mb, colors=['lemonchiffon', 'navajowhite', 'sandybrown'])
        	plt.xlabel('time')
        	plt.ylabel('Mem Usage (MB)')
        	plt.title('Memory Usage')

        	lc_handle = mlines.Line2D([], [], color='lemonchiffon', label='Buffered Memory')
        	nw_handle = mlines.Line2D([], [], color='navajowhite', label='Cached Memory')
        	sb_handle = mlines.Line2D([], [], color='sandybrown', label='Used Memory')

        	lg = plt.legend(frameon=False,handles=[lc_handle, nw_handle, sb_handle])
        	lg_txts = lg.get_texts()
        	plt.setp(lg_txts, fontsize=10)
        	plt_idx += 1
